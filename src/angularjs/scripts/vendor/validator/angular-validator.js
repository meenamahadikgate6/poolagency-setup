      ...
      return function(value, scope, element, attrs) {
        ...
        while (parent.length !== 0) {
          if (parent.hasClass('form-group')) {
			...
			$label = $("<b class='control-label error'>*</b>");
			if (attrs.id) {
              $label.attr('for', attrs.id);
            }
            */
            if ($(element).parent().hasClass('input-group')) {
              //$(element).parent().parent().parent().find("label").prepend($label);
              $(element).parent().parent().parent().find("label").find('b').addClass('error');
              $("#validationError").show();	
			} else {
             	//$(element).parent().parent().find("label").prepend($label);
             	$(element).parent().parent().find("label").find('b').addClass('error')
            	$("#validationError").show();
			}
            break;
          }
